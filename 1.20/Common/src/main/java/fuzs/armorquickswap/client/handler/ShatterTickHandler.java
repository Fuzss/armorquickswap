...
public class ShatterTickHandler {
    ...
    public static EventResult onLivingDeath(LivingEntity entity, DamageSource source) {
        ...
        if (!entity.level().isClientSide || ShatterRenderHandler.containedInBlacklist(entity.getType())) {
    ...
    public static EventResult onLivingTick(LivingEntity entity) {
        ...
        if (!entity.level().isClientSide || !entity.isDeadOrDying() || ShatterRenderHandler.containedInBlacklist(entity.getType())) {
            return EventResult.PASS;
        }
